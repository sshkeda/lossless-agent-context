...
// visible historical `<reasoning>` text blocks so claude can resume without
...
//
// That demotion is intentional, but it would silently leak as plain historical
// reasoning text into a subsequent codex export if importClaudeCodeJsonl
...
// on text that merely contains reasoning-looking tags, and no dependence
// on Claude Code's tolerance for unknown wrapper fields.
...
describe("cross-provider thinking demotion round-trip", () => {
  it("recovers a native codex reasoning item via the sidecar when re-importing a claude session", () => {
    ...
    // Sanity: the seed contains historical reasoning text but no native
    ...
    expect(sidecar.byLineUuid[lineUuid]?.demotedReasoning).toEqual([
      { contentIndex: 0, originalText: codexReasoningText, sourceProvider: "openai-codex", wrapper: "reasoning.v1" },
    ]);

    // Step 2: re-import the claude seed WITH the sidecar. The recovery
    // logic must promote the historical reasoning text back to a
    ...
    expect(reasoningEvents[0]?.kind === "reasoning.created" && reasoningEvents[0].payload.text).toBe(
      codexReasoningText,
    );
    ...
    // The demoted reasoning text must NOT also leak as a text message —
    // promotion replaces, not duplicates.
    expect(assistantTextParts.some((t) => t.includes("<reasoning"))).toBe(false);
    ...
    const codexLines = codexExport
      .split("\n")
      .filter((l) => l.trim())
      .map((l) => JSON.parse(l));
    const reasoningItem = codexLines.find((line) => line?.payload?.type === "reasoning");
    ...
    // No historical reasoning wrapper should leak into any output_text on the codex side.
    for (const line of codexLines) {
      ...
      for (const part of content) {
        if (part?.type === "output_text" && typeof part.text === "string") {
          expect(part.text.includes("<reasoning")).toBe(false);
        }
      }
    }
  });
